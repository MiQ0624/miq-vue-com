          ...
          <template #header>
            <div class="flex justify-between">
              <div>
                ...
                <el-button class="ml-3" @click="openDialog('user-import')"><template #icon><i-ep-upload /></template>导入</el-button>
                <el-button class="ml-3" @click="handleExport"><template #icon><i-ep-download /></template>导出</el-button>
                <el-button class="ml-3" @click="downloadTemplate"><template #icon><i-ep-download /></template>下载模版</el-button>
              </div>
              <div>
                <el-button v-hasPerm="['sys:user:add']" type="success" @click="openDialog('user-form')"><i-ep-plus />新增</el-button>
                <el-button v-hasPerm="['sys:user:delete']" type="danger" :disabled="removeIds.length === 0" @click="handleDelete()"><i-ep-delete />删除</el-button>
              </div>
            </div>
          </template>
          ...
          <el-table v-loading="loading" :data="pageData" @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="avatar" label="头像" align="center">
              <template #default="scope">
                <el-avatar class="el-avatar el-avatar--square" >
                  <el-image style="border:none;cursor: pointer;" :src="getImage(scope.row.avatar)" :preview-src-list="[ `/common/download?name=${scope.row.avatar}` ]">
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" width=40% append-to-body @close="closeDialog">
      ...
      <el-form v-if="dialog.type === 'user-form'" ref="userFormRef" :model="formData" :rules="rules" label-width="100px">
        ...
        <el-form-item label="昵称:" prop="nickname">
          <el-input v-model="formData.nickname" width=40% placeholder="请输入用户昵称" />
        </el-form-item>
